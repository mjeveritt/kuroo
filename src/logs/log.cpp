...
QString Log::init( QObject *myParent )
{
	...
	QString logName = KurooConfig::dirHome() + "/kuroo.log";
	logFile.setName(logName);
	if( !logFile.open(IO_WriteOnly ) ) {
		kdDebug() << i18n("Error writing: ") << KurooConfig::dirHome() << "/kuroo.log" << endl;
		KMessageBox::error(0, i18n("Error writing %1/kuroo.log.").arg(KurooConfig::dirHome()), i18n("Saving"));
		return "";
	}
	else
		return logName;
...
KIO::Job* Log::backupLog()
{
	if ( saveLog && saveLog->isChecked() ) {
		...
		KIO::Job *cpjob = KIO::file_copy( KurooConfig::dirHome() + "/kuroo.log", KurooConfig::dirHome() + "/kuroo_" + dt.toString("yyyyMMdd_hhmm") + ".log", -1, true, false, false );
